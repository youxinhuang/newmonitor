...
var Room = function(){
	...
	_self.showRoomIcon = function(roomData,grd){
		...
		$('<form name="frm" /><br/><b>机房:</b><span name="roomName" id='+'"roomName'+roomData.roomId+'">'+roomData.roomName+'</span>'+'<br/><br/><b>描述:</b><span name="roomDesc" id='+'"roomDesc'+roomData.roomId+'"'+'style="margin-left:15px;">'+roomData.roomDesc+'</span>').appendTo(ctxBody);				
	...
	editroom = function(iddata){
		   var roomDesc=$('#roomDesc'+iddata).text();
		   var roomName=$('#roomName'+iddata).text();
	  	   var r = "机房:<input type='text' id='"+"roomedit"+iddata+"'"+"value="+"'"+roomName+"'"+
	  	   "/><br/>描述:<br/><textarea id='descedit' style='margin-left:20px;'>" +roomDesc+
	  	   "</textarea>";
	      	$('#edit').html(r).dialog({
	      		title:'编辑机房',        	
	      		modal:true,
	      		...
	      		buttons:{
	      			修改:function(){
	      				    var name=$('#roomedit'+iddata).val();
	      				    var desc=$('#descedit').val();
	      				    if(name==roomName&&desc==roomDesc){
	      				    	$(this).dialog('close');
	      				    }else{
		      					var url="/handleRoom.php";
		      					var data="action=updateRoom&roomId="+iddata+"&roomDesc="+desc+"&roomName="+name;
		      					$.post(url,data,function(){
		      						var rn=$('#roomName'+iddata).text(name);
		      						$('#title'+iddata).text(name);
		      						$('#roomDesc'+iddata).text(desc);
		      					});
		      					$(this).dialog('close');
	      				    }
	      			},
